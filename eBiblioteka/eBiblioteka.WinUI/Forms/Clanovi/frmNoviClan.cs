...
namespace eBiblioteka.WinUI.Forms.Clanovi
{
    public partial class frmNoviClan : Form
    {
        ...
        public frmNoviClan(MainForm mainForm)
        {
            InitializeComponent();
            _mainForm = mainForm;
        }
        ...
        ClanInsertRequest request = new ClanInsertRequest();
        ...
        {
            pbSlika.Image = Properties.Resources.clanavatar;
            request.SlikaPutanja = nameof(Properties.Resources.clanavatar) + ".png";
            request.Slika = ImageToByte(Properties.Resources.clanavatar);
        }
        ...
        private async void btnSpremi_Click(object sender, EventArgs e)
        {
            ...
            if (this.ValidateChildren() && ProvjeraValidnostiPolja() == true)
            {
                ...
                else
                {
                    ...
                    request.DatumUcljanjenja = DateTime.Now;

                    var response = await _apiService.Insert<Model.Clan>(request);

                    if (response != null)
                    {
                        MessageBox.Show("Uspješno ste dodali člana.", "Info", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
            }
            else
        ...
        {
            var x = cmbGrad.SelectedIndex;
            var x3 = cmbGrad.SelectedItem;
            var x44 = cmbGrad.SelectedValue;
